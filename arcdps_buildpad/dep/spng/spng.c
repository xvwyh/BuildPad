...
static inline void scale_row(unsigned char *row, uint32_t pixels, int fmt, unsigned depth, struct spng_sbit *sbit)
{
    ...
    else if(fmt == SPNG_FMT_BGRA8)
    {
        ...
        for (i = 0; i < pixels; i++)
        {
            ...
            px[0] = sample_to_target(px[0], depth, sbit->blue_bits, 8);
            px[1] = sample_to_target(px[1], depth, sbit->green_bits, 8);
            px[2] = sample_to_target(px[2], depth, sbit->red_bits, 8);
            px[3] = sample_to_target(px[3], depth, sbit->alpha_bits, 8);
            ...
            for(k=0; k < width; k++)
            {
                ...
                if(ctx->ihdr.color_type == SPNG_COLOR_TYPE_TRUECOLOR)
                {
                    ...
                    else /* == 8 */
                    {
                        ...
                        if(fmt == SPNG_FMT_BGRA8)
                        {
                            memcpy(pixel, scanline + (k * 3) + 2, 1);
                            memcpy(pixel + 1, scanline + (k * 3) + 1, 1);
                            memcpy(pixel + 2, scanline + (k * 3), 1);
                            pixel[3] = 255;
                            continue;
                        }
                ...
                else if(ctx->ihdr.color_type == SPNG_COLOR_TYPE_TRUECOLOR_ALPHA)
                {
                    ...
                    else /* == 8 */
                    {
                        ...
                        if(fmt == SPNG_FMT_BGRA8)
                        {
                            memcpy(row + 0, scanline + 2, 1);
                            memcpy(row + 1, scanline + 1, 1);
                            memcpy(row + 2, scanline + 0, 1);
                            memcpy(row + 3, scanline + 3, 1);
                        }
